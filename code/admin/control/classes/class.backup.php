...
class dbBackup extends db {
  ...
  public function setDatabase($db) {
    $this->database = $db;
    if (!((bool)mysqli_query($GLOBALS['___mysqli_dbcon'], "USE `" . $this->database . "`"))) {
      return false;
    }
    return true;
  }
  ...
  public function getTableStructure($table) {
    ...
    $records = db::db_query('SHOW FIELDS FROM `' . $table . '`');
    if (db::db_rows($records) == 0) {
      return false;
    }
    while ($record = db::db_fetch_assoc($records)) {
    ...
    $structure .= mswDefineNewline() . ")";
    $records = db::db_query("SHOW TABLE STATUS LIKE '" . $table . "'");
    if ($record = db::db_fetch_assoc($records)) {
  ...
  public function mswGetTableData($table, $hexValue = true) {
    ...
    for ($x = 0; $x < $num_fields; $x++) {
      $record = db::db_fetch_assoc($records);
    ...
    $records         = db::db_query($selectStatement);
    $num_rows        = db::db_rows($records);
    $num_fields      = db::db_num_fields($records);
    if ($num_rows > 0) {
      ...
      for ($i = 0; $i < $num_rows; $i++) {
        ...
        for ($j = 0; $j < $num_fields; $j++) {
          $field_name = db::db_field_name($records, $j) -> name;
          if (isset($hexField[$j]) && $hexField[$j] && (strlen($record[$field_name]) > 0)) {
            $data .= "0x" . $record[$field_name];
          } else {
            $data .= "'" . str_replace('\"', '"', (isset($GLOBALS['___mysqli_dbcon']) && is_object($GLOBALS['___mysqli_dbcon']) ? mysqli_real_escape_string($GLOBALS['___mysqli_dbcon'], $record[$field_name]) : '')) . "'";
          }
          $data .= ',';
        }
  ...
  public function getDatabaseStructure() {
    $structure    = '';
    $records      = db::db_query('SHOW TABLES');
    $scriptSchema = mswDBSchemaArray($this);
    if (db::db_rows($records) == 0) {
      return false;
    }
    while ($record = db::db_fetch_rows($records)) {
  ...
  public function getMySQLVersion() {
    $q = @db::db_query("SELECT VERSION() AS v");
    $V = @db::db_object($q);
  ...
  public function doDump() {
    ...
    $header .= '# Music System ' . mswDefineNewline();
    $header .= '# ' . SCRIPT_NAME . ' Version: ' . SCRIPT_VERSION . mswDefineNewline();
    $header .= '# Date Created: ' . $this->datetime->dateTimeDisplay($this->datetime->utcTime(), $this->settings->dateformat) . ' @ ' . $this->datetime->dateTimeDisplay($this->datetime->utcTime(), $this->settings->timeformat) . mswDefineNewline();
  ...
  public function getSqlKeysTable($table) {
    ...
    $index           = array();
    $fulltext        = array();
    $results         = db::db_query("SHOW KEYS FROM `{$table}`");
    if (db::db_rows($results) == 0) {
      return false;
    }
    while ($row = db::db_object($results)) {
