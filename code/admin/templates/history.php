...
$SQL   = 'WHERE (`'.DB_PREFIX.'sales`.`account` = \''.$ID.'\') AND `'.DB_PREFIX.'sales`.`enabled` = \'yes\' ';
$FLTR  = 'Order by `'.DB_PREFIX.'sales`.`ts` DESC';
if (isset($_GET['gw'])) {
  if (substr($_GET['gw'],0,3)=='st-') {
    $SQL = "AND (`status` = '".mswSafeString(urldecode(substr($_GET['gw'],3)), $DB)."')";
  } else {
    $SQL = "AND (`gateway` = '".mswSafeString(urldecode($_GET['gw']), $DB)."')";
  }
}
if (isset($_GET['f'])) {
  switch ($_GET['f']) {
        ...
        <div id="page-wrapper">
                ...
                <div class="col-lg-12">
                    <h1 class="page-header">
                     <span style="float:right">
                      ...
                      if ($r > 0) {
                      ?>
                      <button type="button" class="btn btn-primary btn-sm" onclick="mm_windowLoc('export-sales&amp;ph=<?php echo $ID; ?>')" title="<?php echo mswSafeDisplay($adlang9[11]); ?>"><i class="fa fa-save fa-fw"></i></button>
            ...
            <div class="row">
                <div class="col-lg-12">
                                    ...
                                    while ($S = $DB->db_object($Q)) {
                                      ...
                                      ?>
                                            ...
                                            <td>
                                             ...
                                             <a href="?p=new-sale&amp;edit=<?php echo $S->saleID; ?>&amp;his=<?php echo $ID; ?>" title="<?php echo mswSafeDisplay($gblang[12]); ?>"><i class="fa fa-pencil fa-fw"></i></a>
					...
					<?php
                    $PTION = new pagination(array($r,$gblang[15],$page),'?p='.$_GET['p'].'&amp;id='.$_GET['id'].'&amp;next=');
                    echo $PTION->display();
                    ?>
				</div>
			</div>
