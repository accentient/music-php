...
if (!isset($GW->id) || !file_exists(PATH . 'control/classes/gateways/' . $GW->class)) {
  die('Gateway not found, callback cancelled.');
  exit;
}
...
if ($CALLBACK['code-id']) {
  ...
  if (isset($SALE_ORDER->saleID) && $MARKER == 'mswmusic') {
    ...
    $GATEWAY->order['rcode'] = $CALLBACK['signature'];
    ...
    if ($GATEWAY->validate() == 'ok') {
      ...
      if ($SALE_ORDER->saleEnabled == 'no') {
        ...
        switch ($paymentStatus) {
          ...
          case 'completed':
            if (($CALLBACK['amount'] >= str_replace('.', '', $saleTotal)) && ($CALLBACK['currency'] == $SETTINGS->currency)) {
          ...
          default:
            ...
            if (isset($_POST['responseMessage'])) {
              $GATEWAY->log($SALE_ID, 'Gateway message: ' . $_POST['responseMessage']);
            }
            break;
        }
      } else {
