...
if ($CALLBACK['code-id']) {
  ...
  if (isset($SALE_ORDER->saleID) && $MARKER == 'mswmusic') {
    ...
    if ($GATEWAY->validate() == 'ok') {
      ...
      if ($paymentStatus == 'fraud_status_changed' && strtolower($CALLBACK['fraud-status']) == 'pass') {
        $paymentStatus = 'completed';
      }

      // ADJUST FOR PENDING SALE (ORDER CREATED)..
      if ($paymentStatus == 'order_created' && strtolower($CALLBACK['fraud-status']) == 'wait' && strtolower($CALLBACK['inv-status']) == 'approved') {
        if ($SETTINGS->propend == 'yes') {
          $paymentStatus = 'completed';
        } else {
