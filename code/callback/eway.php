...
if (isset($_POST['AccessPaymentCode'])) {
  $order = $GATEWAY->pingcallback();
  if ($order[0] > 0 && ctype_alnum($order[1])) {
    header("Location: " . BASE_HREF . "index.php?gw=" . $order[0] . '-' . $order[1]);
  } else {
    header("Location: " . BASE_HREF . "index.php?p=error&msg=Callback failure");
  }
  exit;
}
    ...
    if ($GATEWAY->validate() == 'ok') {
      ...
      if ($SALE_ORDER->saleEnabled == 'no') {
        ...
        switch ($paymentStatus) {
          ...
          // NOT currently supported by eWay
          //==========================================
...
// For eway throw error page if we are this point..
header('Location: ' . $SETTINGS->httppath . '?msg=1');
