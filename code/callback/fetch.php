...
if (!isset($GW->id)) {
  die('Gateway data not found for "' . $thisGateway . '", callback terminated!');
  exit;
}
    ...
    if ($GATEWAY->validate() == 'ok') {
      ...
      if ($SALE_ORDER->saleEnabled == 'no') {
        ...
        switch ($paymentStatus) {
          ...
          // NOT currently supported by Fetch
          //==========================================
          ...
          default:
            ...
            if (isset($_POST['response_text'])) {
              $GATEWAY->log($SALE_ID, 'Gateway message: ' . $_POST['response_text']);
            }
            break;
        }
        ...
        if ($_POST['mc_gross'] > 0) {
          ...
          // DEBUG..
          $GATEWAY->log($SALE_ID, 'Received callback for previously processed sale where amount is less than 0.00. Possible refund requiring acceptance by buyer.');
