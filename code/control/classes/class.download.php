...
class downloads {
  ...
  public function dl($path, $mime, $delete = 'yes', $id = 0, $data = array()) {
    $newname = $path;
    ...
    // Is it a tmp file?
    // Force zip extension..
    if (substr(strtolower($path), -4) == '.tmp' || strpos($path, '.') === false) {
      $newname = (isset($data[0]) ? mswSaleInvoiceNumber($data[0]) : rand(99, 99999)) . '-' . time() . '.zip';
    ...
    // For some extensions, we don`t need this..
    // Can cause some browser anomalies..
    if (!in_array($mime, array('application/zip'))) {
    ...
    if ($id > 0) {
      $this->log('Item: ' . $id . ' - Download completed', 'yes');
    }
    exit;
  }
  ...
  public function log($txt, $last = 'no') {
    if ($this->debuglog == 'yes' && is_dir(PATH . GW_LOG_FOLDER_NAME) && is_writeable(PATH . GW_LOG_FOLDER_NAME)) {
      $message = strtoupper(SCRIPT_NAME) . ' DOWNLOAD LOG @ ' . date("j F Y H:i:s") . mswDefineNewline();
      $message .= 'Action/Info: ' . $txt . mswDefineNewline();
      if ($last == 'yes') {
        $message .= mswDefineNewline() . '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =';
        $message .= mswDefineNewline() . '= = = = = = = = = = = = = = = END OF LOG ENTRY = = = = = = = = = = = = = = = =';
      }
      ...
      if (is_dir(PATH . GW_LOG_FOLDER_NAME)) {
        @file_put_contents(PATH . GW_LOG_FOLDER_NAME . '/download-log.log', $message, FILE_APPEND);
      }
    }
  }
